...
class ModelExecutor:
    ...
    def misclassification_cost(self, y_test, y_pred):
        _, w_fp, w_fn, _ = ModelExecutor.get_weighted_predictions(y_test, y_pred, self.class_weights)
        return (w_fp + w_fn)/len(y_test)

    def weighted_accuracy(self, y_test, y_pred):
        w_tn, w_fp, w_fn, w_tp = ModelExecutor.get_weighted_predictions(y_test, y_pred, self.class_weights)
        return (w_tp+w_tn)/(w_tn+ w_fp+ w_fn+ w_tp)

    def weighted_f1_score(self, y_test, y_pred):
        _, w_fp, w_fn, w_tp = ModelExecutor.get_weighted_predictions(y_test, y_pred, self.class_weights)
    ...
    def _create_scorers(self):
        if self._task=="regression":
            ...
            self.metric_collection = ModelExecutor.convert_metrics_to_scorers(_CLASSIFICATION_METRICS)
