...
def dummy_df_test():
    ...
    ohe_value_list = [num for num in range(OHE_MAX_VAL)]
    ...
    embedding_value_list = [num for num in range(EMBEDDING_MAX_VAL)]
    test_df[embedding_col_names] = np.random.choice(embedding_value_list, (NUM_ROWS, NUM_EMBEDDING_COLS))

    test_df = map_cat_values(test_df, embedding_col_names)
    cat_value_cols = [col for col in test_df.columns if col.endswith("_val")]
    val_cols = cat_value_cols+embedding_col_names+ohe_col_names+continuous_cols
    random_weights = [np.random.random() for col in val_cols]
    ...
    assert mle_regression.results[best_regression_model]["r2"] > 0.95
